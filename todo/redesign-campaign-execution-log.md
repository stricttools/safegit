...
- 44 changelog entries covering all 398 non-exempt commits in
  v0.28.0..HEAD (413 total, 15 Autogenerated-exempt): 11 breaking, 13
  feature, 12 fix, 8 no-user-facing clusters; 16 batch exclusions with
  purpose-written reasons; one deliberate dual-entry commit (9a8272e:
  breaking + feature). rlsbl check --tag changelog 12/12 PASS; tree
  clean.
- Ratified: the writer's one surgical hand-edit of unreleased.jsonl
  (repairing its own early line-shift error — the first adds used
  unpinned git-log line numbers while each add's auto-commit shifted
  the range; rlsbl exposes no entry deletion or commit-list edit; the
  repair was committed, re-added against a pinned range, and every
  user-facing entry audited commit-by-commit afterwards); the dry-run
  uninstall fix riding inside the breaking uninstall entry (one
  commit, both aspects stated); docs healing as ONE user-facing fix
  entry (34 commits — it corrected instructions users follow); 36
  user-facing entries standing (the 15-25 figure was an estimate, not
  a cap; strict feature-area grouping governs); uninstall and the
  exit-code changes classified breaking on the writer's judgment
  (correct).

PHASE 10 IS COMPLETE (10.1 green, 10.2 audited and remediated across
four waves, 10.3 covered and validated). Remaining: the user's
divergence-document review (pre-release), then Phase 11 (triage,
release file, the single release).
