...
type AmendRequest struct {
	...
	// AllowNonPortableTargets is the same input CommitRequest carries: the
	// election to record a symlink whose target text will not resolve in another
	// checkout, which intake refuses without it.
...
func (p *Pipeline) Amend(ctx context.Context, req AmendRequest) (*AmendResult, error) {
	...
	// content.
	//
	// The base is hard-coded here, and the exemption needs the base as well as the
	// context, so a declared sequencer context can no longer carry an amend past
	// this guard: an amend is never a conclusion.
	...
	files, err := p.resolveFiles(ctx, repoRoot, baseRev(ctx, ref), req.FileSpecs, req.Untrack, req.AllowNonPortableTargets)
	if err != nil {
		return nil, err
	}
...
func (p *Pipeline) Reword(ctx context.Context, req RewordRequest) (*RewordResult, error) {
	...
	//
	// The base is hard-coded here, and the exemption needs the base as well as the
	// context, so a declared sequencer context can no longer carry a reword past
	// this guard: a reword is never a conclusion.
	if err := guardUnmergedIndex(ctx, req.Sequencer, IndexBaseParentTree); err != nil {
		return nil, err
	}
