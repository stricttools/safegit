...
14. **Reconcile the shared index** with the new commit, preserving staged work the parent tip does not account for -- on the paths this operation did not stage. A path it did stage holds what the commit recorded there and nothing else, so a `git mv` staged before the file was edited, or a `git rm --cached` of a path the commit re-adds, cannot be replayed back over the commit's own answer. Only when committing to the checked-out branch: a cross-branch commit must not touch this worktree's index at all.
...
**Release is identity-checked too.** A holder removes the path only while it still names the exact file it published: `SameFile` against the stat recorded at publication time, AND the owner record on disk byte-for-byte against the one it wrote. Both halves are needed. A stat identity is dev plus inode, and a filesystem is free to hand a freed inode number straight back out -- ext4 does, btrfs does not -- so after a force-release the newcomer's lock can occupy the very inode this one had and `SameFile` alone would answer "ours" about somebody else's live lock. A lock file is immutable once published, so the record settles what the inode number cannot. The reachable case that makes any of this necessary: an operator force-releases a lock this process still holds, a third process wins the free path and publishes its own live lock there, and the original process then finishes -- a blind `unlink` would delete the newcomer's lock while the newcomer is working. A mismatch means our lock is already gone, so there is nothing to remove and nothing to report.
...
- **`safegit commit --moved 'old -> new'`** states that a move already happened: the old path must be tracked in the commit's parent and gone from disk, the new one must be present. A declaration the repository does not bear out exits **19** and commits nothing. The commit's OWN TREE is asked the same two questions once it is written -- the new path in it, the old path gone from it -- so a declaration can never ride on a commit that carries neither side of it. That check is not a licence to stage: a destination the caller did not name (or one a directory expansion passed over as gitignored) is refused, naming the path to add to the file list, because safegit stages what the caller names and nothing else.
