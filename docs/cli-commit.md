...
| `--allow-escaping-targets`, `--no-allow-escaping-targets` |  | bool | optional |  | record a symlink whose target resolves outside the repository, which the commit stores as the link text; omitted, and with --no-allow-escaping-targets, such a link is refused with its target named, because in another checkout it resolves to nothing or to a file the repository never carried |
